...
class PXRFConverter(CrossreadsPetrographyTool):
    ...
    def get_description(self, isic, isic_letter):
        ...
        return (
            "; ".join(df[isic_letter].dropna().apply(str).unique()) if len(df) else ""
        )
    ...
    def df_input(self):
        return pd.DataFrame(
            [{"filename": fn, "text": txt} for fn, txt in self.txt_input]
        )
    ...
    def df_standards_parsed(self, verbose=False):
        ...
        for filename, txt in txts:
            ...
            for srctxt in txt.strip().split("\n\n"):
                ...
                if verbose:
                    print([filename, src, is_standard, standard_key])
        ...
        df["standard_group"] = [
            get_standard_group(element, row)
            for element, row in df.fillna("").iterrows()
        ]
    ...
    def get_df_adjusted(self, verbose=False, agg_stdev=False):
        ...
        for filename, txt in txts:
            ...
            for srctxt in txt.strip().split("\n\n"):
                ...
                if is_mk:
                    ca_si = (
                        df.loc["CaO"]["Mass_fraction"] / df.loc["SiO2"]["Mass_fraction"]
                    )
                ...
                df["Calc_fraction"] = df["Calc_fraction"].apply(
                ...
                try:
                    isics = [
                        subsrc
                        for subsrc in src.split("-")
                        if subsrc.startswith("ISic") or subsrc.startswith("Isic")
                    ]
                    isic = (
                        isics[0].strip().replace("ISic", "Isic")
                        if isics
                        else src.split("-")[0].strip()
                    )
                    if not "Isic" in isic:
                    ...
                    df["desc_category"] = self.get_description_category(
                        isic, isic_letter
                    )
                    # if not is_mk:
                    #     print(desc)
                    ...
                    logger.warning(
                        f"[{filename}] Could not find {repr(isic_letter)} in logbook columns ({list(desc_rows.columns)})"
                    )
                    desc = "?"
            ...
            .agg(
                {
                    "desc": "first",
                    **{
                        col: "mean" if not agg_stdev else "std"
                        for col in odf.columns
                        if col != "desc" and pd.api.types.is_numeric_dtype(odf[col])
                    },
                }
            )
            .reset_index()
        ...
        for g, gdf in odf.groupby(["Isic", "desc_category"]):
            ...
            sumval = sum(
                d.get("Calc_fraction", 0) for d in gld if d.get("Element") in els
            )
            for d in gld:
                if d.get("Element") in els:
                    d["Calc_fraction"] = (
                        d["Calc_fraction"] / sumval * 100 if sumval else 0
                    )
                ld.append(d)
        odf = pd.DataFrame(ld)
        return odf
    ...
    def plot(self):
        ...
        fig = p9.ggplot(
            df.reset_index(),
            p9.aes(x="Mass_fraction", y="standard_val", color="standard_group"),
        )
        fig += p9.geom_point()
    ...
    def save(self, output_folder=None, pivot_table=True):
        ...
        if pivot_table:
            df_mean = df_mean.pivot(
                index=["Isic", "desc_category"],
                columns="Element",
                values="Calc_fraction",
            ).reset_index()
            df_std = df_std.pivot(
                index=["Isic", "desc_category"],
                columns="Element",
            ...
            df_mean = df_mean[
                [c for c in prefcols if c in df_mean]
                + [c for c in df_mean if c not in prefcols]
            ]
            df_std = df_std[
                [c for c in prefcols if c in df_std]
                + [c for c in df_std if c not in prefcols]
            ]
