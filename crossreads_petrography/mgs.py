...
class MgsConverter(CrossreadsPetrographyTool):
    ...
    def df_ranges(self):
        ...
        logger.debug(
            f"Converted 'value_mm' to numeric. Null values: {df_mgs['value_mm'].isnull().sum()}"
        )
        ...
        df_ranges = df_mgs.pivot(
            index="subtype", columns="value_type", values="value_mm"
        )
    ...
    def df_output(self):
        ...
        df = pd.DataFrame(
            index=read_metadata(metamorphic=True).index.dropna().unique(),
            columns=df_ranges.index,
        )
                ...
                for subtype, row in df_ranges.iterrows():
                    ...
                    if intersection:
                        ...
                        if subtype not in df.columns:
                            df[subtype] = ""
                        val = df.loc[isic,subtype]
                        valcount = val.count(symbols[coltype])
                        if valcount < 2:
                            df.loc[isic, subtype] = df.loc[isic, subtype] + intersection
        ...
        df["has_optical"] = df.apply(
            lambda row: symbols["optical"] in "".join(row), axis=1
        )
        logger.debug(
            f"Added 'has_optical' column. True count: {df['has_optical'].sum()}"
        )
